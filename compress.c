...
int interface_in, interface_out;
...
struct buffer_ops * create_file_buffer_ops(const char *file_name, const char *mode, int id) {
...
int file_encode(const char *input_file, const char *output_file) {
    ...
    in = create_file_buffer_ops(input_file, "rb" , 0);
    if (!in) {
        LOGE("Create input file ops failed\n");
        return 1;
    }
    out = create_file_buffer_ops(output_file, "wb", 0);
    if (!out) {
        LOGE("Create input file ops failed\n");
        goto OUT;
    }
...
int file_decode(const char *input_file, const char *output_file) {
    ...
    in = create_file_buffer_ops(input_file, "rb" , 1);
    if (!in) {
        LOGE("Create input file ops failed\n");
        return 1;
    }
    out = create_file_buffer_ops(output_file, "wb" , 2);
    if (!out) {
        LOGE("Create input file ops failed\n");
        goto OUT;
    }
...
char in_huffman[99], out_huffman[99];
//char *input, *output;
//end huffman defined
...
nextarg:	continue;
      		...
      		for (fileptr = filelist; *fileptr; fileptr++)
					{
						  if(do_decomp == 0)
							...
							{
								char in_huffman[99] = {};
								strcpy(in_huffman, *fileptr);
								char out_huffman[99] = {};
								strncpy(out_huffman, in_huffman, strlen(in_huffman)-2);
								file_decode(in_huffman, out_huffman);
								sleep(0.5);
								strcpy(*fileptr, out_huffman);
								comprexx(*fileptr);
							}
					}
    	}
    ...
    FD_ZERO(&reads);
    FD_ZERO(&writes);
    FD_SET(interface_in, &reads);
    FD_SET(interface_out, &writes);
    timeout.tv_sec = 1;
		timeout.tv_usec = 500;
    switch( select(2, &reads, &writes, NULL, &timeout))
    {
      case 0:
      ...
      case -1:
        ...
        break;
      default:
        if(FD_ISSET(interface_out, &reads))
        {
          //printf("writing hasn't done");
					sleep(0.5);
        }
      break;
    }
		//printf("can start huffman\n");
		//interface end

		//start huffman
		//printf("do_decomp is %d\n", do_decomp);
		if(do_decomp == 0)
		{
			char out_huffman[99];
			strcpy(out_huffman, in_huffman);
			strcat(out_huffman,".H");
			file_encode(in_huffman, out_huffman);
		}
		//end huffman
		remove(in_huffman);
